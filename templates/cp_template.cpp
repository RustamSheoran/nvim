...
template <class T>
istream& operator>>(istream& input, vector<T>& values) {
    for (T& value : values) input >> value;
    return input;
}
...
struct DSU {
    ...
    int find(int vertex) { return parent[vertex] == vertex ? vertex : parent[vertex] = find(parent[vertex]); }
...
struct WeightedDSU {
    ...
    pair<int, ll> find(int vertex) {
        if (parent[vertex] == vertex) return {vertex, 0};
    ...
    bool merge(int first, int second, ll difference) {
        ...
        if (root_first == root_second) return value_second - value_first == difference;
    ...
    optional<ll> diff(int first, int second) {
        auto [root_first, value_first] = find(first);
        auto [root_second, value_second] = find(second);
        if (root_first != root_second) return nullopt;
...
struct Fenwick {
    ...
    void build(const vector<T>& values) {
        ...
        for (int index = 0; index < n; index++) {
            ...
            if (next <= n) bit[next] += bit[index + 1];
        }
    }
    ...
    int lower_bound(T target) const {
        ...
        for (int step = 1 << bit_width((unsigned)n); step; step >>= 1)
            if (index + step <= n && bit[index + step] < target) target -= bit[index += step];
        return index;
    }
};
...
struct Fenwick2D {
    ...
    void update(int row, int column, T delta) {
        for (++row; row <= n; row += row & -row)
            for (int current = column + 1; current <= m; current += current & -current) bit[row][current] += delta;
    }
    T query(int row, int column) const {
        T result{};
        for (; row > 0; row -= row & -row)
            for (int current = column; current > 0; current -= current & -current) result += bit[row][current];
        return result;
    }
    T query(int top, int left, int bottom, int right) const {
        return query(bottom, right) - query(top, right) - query(bottom, left) + query(top, left);
    }
};
...
struct IterativeSegmentTree {
    ...
    Merge merge;
    vector<T> tree;
    IterativeSegmentTree(int n_, T identity_, Merge merge_) : identity(identity_), merge(merge_) { init(n_); }
    void init(int n_) {
        n = 1;
        while (n < n_) n <<= 1;
    ...
    void build(const vector<T>& values) {
        init((int)values.size());
        for (int index = 0; index < (int)values.size(); index++) tree[n + index] = values[index];
        for (int index = n - 1; index; index--) tree[index] = merge(tree[index << 1], tree[index << 1 | 1]);
    }
    void update(int index, T value) {
        for (tree[index += n] = value; index >>= 1;) tree[index] = merge(tree[index << 1], tree[index << 1 | 1]);
    }
    T query(int left, int right) const {
        T first = identity, second = identity;
        for (left += n, right += n; left < right; left >>= 1, right >>= 1) {
            if (left & 1) first = merge(first, tree[left++]);
            if (right & 1) second = merge(tree[--right], second);
        }
...
struct LazySegmentTree {
    ...
    void update(int left, int right, ll value) { update(1, 0, n, left, right, value); }
    ...
    ll query(int node, int start, int finish, int left, int right) {
        ...
        push(node, start, finish);
        int middle = (start + finish) / 2;
        return query(node << 1, start, middle, left, right) + query(node << 1 | 1, middle, finish, left, right);
    }
};
...
struct SparseTable {
    ...
    Merge merge;
    SparseTable(const vector<T>& values, Merge merge_) : merge(merge_) { build(values); }
    void build(const vector<T>& values) {
        ...
        for (int level = 1; (1 << level) <= n; level++)
            for (int index = 0; index + (1 << level) <= n; index++)
                table[level][index] = merge(table[level - 1][index], table[level - 1][index + (level - 1)]);
    }
    T query(int left, int right) const {
...
struct LiChaoTree {
    ...
    vector<Line> tree;
    vector<bool> used;
    LiChaoTree(ll left_, ll right_) : left(left_), right(right_), tree(4), used(4, false) {}
    ...
    void add_line(int node, ll start, ll finish, Line line) {
        ...
        bool low = line.value(start) < tree[node].value(start), mid = line.value(middle) < tree[node].value(middle);
        if (mid) swap(line, tree[node]);
    ...
    ll query(int node, ll start, ll finish, ll x) const {
        if (node >= (int)tree.size()) return INFLL;
        ll result = used[node] ? tree[node].value(x) : INFLL;
        if (start == finish) return result;
        ll middle = start + (finish - start) / 2;
        return min(result,
                   x <= middle ? query(node << 1, start, middle, x) : query(node << 1 | 1, middle + 1, finish, x));
    }
};
...
vi bfs(const vector<vi>& graph, int source) {
    ...
    while (!queue.empty()) {
        ...
        for (int next : graph[vertex])
            if (distance[next] == INF) distance[next] = distance[vertex] + 1, queue.push(next);
    }
    return distance;
}
...
vll dijkstra(const vector<vector<pair<int, ll>>>& graph, int source) {
    ...
    while (!queue.empty()) {
        ...
        for (auto [next, weight] : graph[vertex])
            if (distance[next] > current + weight) queue.push({distance[next] = current + weight, next});
    }
    return distance;
}
...
optional<vi> topological_sort(const vector<vi>& graph) {
    ...
    for (const auto& edges : graph)
        for (int next : edges) degree[next]++;
    queue<int> queue;
    for (int vertex = 0; vertex < n; vertex++)
        if (!degree[vertex]) queue.push(vertex);
    while (!queue.empty()) {
        int vertex = queue.front();
        queue.pop();
        order.PB(vertex);
...
bool has_directed_cycle(const vector<vi>& graph) {
    ...
    auto dfs = [&](auto&& self, int vertex) -> bool {
        ...
        for (int next : graph[vertex]) {
            ...
            if (!state[next] && self(self, next)) return true;
        }
        state[vertex] = 2;
        return false;
    };
    for (int vertex = 0; vertex < (int)graph.size(); vertex++)
        if (!state[vertex] && dfs(dfs, vertex)) return true;
    return false;
}
...
pair<vll, bool> bellman_ford(int n, const vector<Edge>& edges, int source) {
    ...
    for (int iteration = 0; iteration < n; iteration++) {
        ...
        if (!changed) return {distance, false};
    }
...
pair<ll, vector<Edge>> kruskal(int n, vector<Edge> edges) {
    ...
    for (const Edge& edge : edges)
        if (dsu.merge(edge.from, edge.to)) cost += edge.weight, tree.PB(edge);
    return {cost, tree};
}
...
pair<ll, vi> prim(const vector<vector<pair<int, ll>>>& graph, int source = 0) {
    ...
    while (!queue.empty()) {
        ...
        for (auto [next, edge_weight] : graph[vertex])
            if (distance[next] > edge_weight) queue.push({distance[next] = edge_weight, next}), parent[next] = vertex;
    }
    return {cost, parent};
}
        ...
        auto dfs = [&](auto&& self, int vertex) -> void {
            ...
            if (low[vertex] == index[vertex]) {
                while (true) {
                    ...
                    if (next == vertex) break;
                }
                components++;
            }
        };
        for (int vertex = 0; vertex < n; vertex++)
            if (index[vertex] == -1) dfs(dfs, vertex);
    }
};
...
struct LCA {
        ...
        for (int index = 0; index < (int)order.size(); index++)
            for (int next : tree[order[index]])
                if (next != parent[order[index]])
                    parent[next] = order[index], depth[next] = depth[order[index]] + 1, order.PB(next);
        up[0] = parent;
        for (int level = 1; level < log; level++)
            for (int vertex = 0; vertex < n; vertex++) up[level][vertex] = up[level - 1][up[level - 1][vertex]];
    }
    int jump(int vertex, int steps) const {
        for (int level = 0; steps; level++, steps >>= 1)
            if (steps & 1) vertex = up[level][vertex];
        return vertex;
    }
    int query(int first, int second) const {
        ...
        for (int level = log - 1; level >= 0; level--)
            if (up[level][first] != up[level][second]) first = up[level][first], second = up[level][second];
        return up[0][first];
    }
};
...
struct Dinic {
    ...
    void add_edge(int from, int to, ll capacity) {
        FlowEdge forward{to, (int)graph[to].size(), capacity}, backward{from, (int)graph[from].size(), 0};
    ...
    bool bfs(int source, int sink) {
        ...
        while (!queue.empty()) {
            ...
            for (const auto& edge : graph[vertex])
                if (edge.capacity && level[edge.to] == -1) level[edge.to] = level[vertex] + 1, queue.push(edge.to);
        }
        return level[sink] != -1;
    }
    ll dfs(int vertex, int sink, ll pushed) {
        if (!pushed || vertex == sink) return pushed;
        for (int& edge_index = pointer[vertex]; edge_index < (int)graph[vertex].size(); edge_index++) {
            FlowEdge& edge = graph[vertex][edge_index];
            if (level[edge.to] != level[vertex] + 1 || !edge.capacity) continue;
    ...
    ll max_flow(int source, int sink) {
        ...
        while (bfs(source, sink)) {
            ...
            while (ll flow = dfs(source, sink, INFLL)) result += flow;
        }
        return result;
    }
};
...
vi prefix_function(const string& text) {
    ...
    for (int index = 1; index < (int)text.size(); index++) {
        ...
        while (border && text[index] != text[border]) border = pi[border - 1];
        if (text[index] == text[border]) border++;
        pi[index] = border;
    }
    return pi;
}
...
vi kmp_find(const string& text, const string& pattern) {
    ...
    for (int index = (int)pattern.size() + 1; index < (int)pi.size(); index++)
        if (pi[index] == (int)pattern.size()) positions.PB(index - 2 * (int)pattern.size());
    return positions;
}
...
vi z_function(const string& text) {
    ...
    for (int index = 1; index < n; index++) {
        if (index < right) z[index] = min(right - index, z[index - left]);
        while (index + z[index] < n && text[z[index]] == text[index + z[index]]) z[index]++;
        if (index + z[index] > right) left = index, right = index + z[index];
    }
    if (n) z[0] = n;
    return z;
}
...
struct Trie {
    ...
    void insert(const string& word) {
        ...
        for (char character : word) {
            ...
            if (nodes[vertex].next[index] == -1) nodes[vertex].next[index] = (int)nodes.size(), nodes.PB(Node());
...
pair<vi, vi> manacher(const string& text) {
    ...
    for (int type = 0; type < 2; type++) {
        ...
        int left = 0, right = -1;
        for (int index = 0; index < n; index++) {
            int value = index > right ? 0 : min(radius[left + right - index + type], right - index + 1);
            while (index + value < n && index - value - 1 + type >= 0 &&
                   text[index + value] == text[index - value - 1 + type])
                value++;
            radius[index] = value;
            if (index + value - 1 > right) left = index - value + type, right = index + value - 1;
        }
    }
    return {odd, even};
}
...
ll extended_gcd(ll first, ll second, ll& x, ll& y) {
    if (!second) return x = 1, y = 0, first;
    ll gcd_value = extended_gcd(second, first % second, y, x);
    y -= first / second * x;
    return gcd_value;
}

optional<pair<ll, ll>> crt(ll first_remainder, ll first_modulus, ll second_remainder, ll second_modulus) {
    ll x, y, divisor = extended_gcd(first_modulus, second_modulus, x, y);
    if ((second_remainder - first_remainder) % divisor) return nullopt;
    ll lcm_value = first_modulus / divisor * second_modulus;
    ll value = (first_remainder + (__int128)(second_remainder - first_remainder) / divisor * x %
                                      (second_modulus / divisor) * first_modulus) %
               lcm_value;
    if (value < 0) value += lcm_value;
...
vi linear_sieve(int limit) {
    ...
    for (int value = 2; value <= limit; value++) {
        if (!least_prime[value]) least_prime[value] = value, primes.PB(value);
        for (int prime : primes) {
            if (prime > least_prime[value] || value * prime > limit) break;
...
vector<pair<ll, int>> prime_factorization(ll value) {
    ...
    for (ll divisor = 2; divisor * divisor <= value; divisor += 1 + (divisor > 2))
        if (value % divisor == 0) {
            int count = 0;
            while (value % divisor == 0) value /= divisor, count++;
            factors.PB({divisor, count});
        }
    if (value > 1) factors.PB({value, 1});
    return factors;
}

template <class T>
vi coordinate_compress(vector<T>& values) {
    vector<T> sorted = values;
    sort(sorted.begin(), sorted.end());
    sorted.erase(unique(sorted.begin(), sorted.end()), sorted.end());
    vi result(values.size());
    for (int index = 0; index < (int)values.size(); index++)
        result[index] = lower_bound(sorted.begin(), sorted.end(), values[index]) - sorted.begin();
    values = move(sorted);
    return result;
}

template <class T>
vector<T> prefix_sum(const vector<T>& values) {
    vector<T> prefix(values.size() + 1);
    for (int index = 0; index < (int)values.size(); index++) prefix[index + 1] = prefix[index] + values[index];
    return prefix;
}
...
vector<T> difference_array(int n, const vector<tuple<int, int, T>>& updates) {
    ...
    for (auto [left, right, value] : updates) difference[left] += value, difference[right] -= value;
    for (int index = 1; index < n; index++) difference[index] += difference[index - 1];
    difference.popb();
    return difference;
}
